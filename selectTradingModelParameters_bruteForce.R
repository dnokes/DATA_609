...
strategyObjectiveFunctionAtrMultiplier<-function(atrMultiplier){
...
for (slowLookbackIndex in  1:nSlowLookback){
  ...
  for (fastLookbackIndex in 1:nFastLookback){
    ...
    if (fastLookback<slowLookback){
      performance[fastLookbackIndex,slowLookbackIndex]<-strategyObjectiveFunction(fastLookback,
        slowLookback)
    }
  }
}

persp(fastLookbackRange,slowLookbackRange,performance,
  phi = 45, theta = 205,xlab='Fast EMA Lookback',
  ylab='Slow EMA Lookback',
  main='Geometric Mean By Trend Filter Lookbacks',
  zlab='Geometric Mean')

contour(fastLookbackRange,slowLookbackRange,performance,
  xlab='Fast EMA Lookback',ylab='Slow EMA Lookback',
  main='Geometric Mean By Trend Filter Lookbacks')


maxTwrIndex<-performance==max(performance,na.rm=TRUE)
...
minAtrMultiplier<-1
maxAtrMultiplier<-10
atrMultiplierRange<-seq(minAtrMultiplier,maxAtrMultiplier)
nAtrMultipliers<-length(atrMultiplierRange)

performanceAtrMultiplier<-matrix(NA,nAtrMultipliers,1)

for (atrMultiplierIndex in 1:nAtrMultipliers){
  #
  atrMultiplier<-atrMultiplierRange[atrMultiplierIndex]
  #
  performanceAtrMultiplier[atrMultiplier]<-strategyObjectiveFunctionAtrMultiplier(atrMultiplier)


}

plot(performanceAtrMultiplier,type='l',main='Geometric Mean By ATR Multiplier',
  xlab='ATR Multiplier',ylab='Geometric Mean')
